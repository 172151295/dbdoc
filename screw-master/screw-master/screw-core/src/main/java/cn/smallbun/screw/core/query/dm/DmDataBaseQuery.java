...
public class DmDataBaseQuery extends AbstractDatabaseQuery {
    ...
    public DmDataBaseQuery(DataSource dataSource, boolean includeView) {
        ...
        try {
            ...
            if (table.equals(PERCENT_SIGN)) {
                if (CollectionUtils.isEmpty(columnsCaching)) {
                    ...
                    List<DmColumnModel> inquires = Mapping.convertList(resultSet,
                        DmColumnModel.class);
            ...
            //单表查询：未缓存过该表时执行单表列查询，支持多次逐表调用（指定表导出场景）
                ...
                //单表结果全部属于该表，直接按表名缓存（避免把其他表缓存为空列表）
