...
public class OracleDataBaseQuery extends AbstractDatabaseQuery {
    ...
    public OracleDataBaseQuery(DataSource dataSource, boolean includeView) {
    ...
    public List<OracleTableModel> getTables() throws QueryException {
        ...
        try {
            ...
            //获取所有表 查询表名、说明
            String tableTypeFilter = " TABLE_TYPE = 'TABLE'";
            if (includeView) {
                tableTypeFilter = " TABLE_TYPE IN ('TABLE', 'VIEW')";
            }
            String sql = "SELECT TABLE_NAME,COMMENTS AS REMARKS FROM USER_TAB_COMMENTS WHERE"
                         + tableTypeFilter;
            if (isDda()) {
                //DBA 使用 DBA_TAB_COMMENTS 进行查询 Oracle连接用户和schema不同问题。dba连接用户可以生成不同schema下的表结构
                sql = "SELECT TABLE_NAME,COMMENTS AS REMARKS FROM DBA_TAB_COMMENTS WHERE"
                      + tableTypeFilter + " AND OWNER = '" + getSchema() + "'";
            }
            ...
            //仅关闭结果集，不关闭共享连接（指定表导出逐表查询时连接需复用）
