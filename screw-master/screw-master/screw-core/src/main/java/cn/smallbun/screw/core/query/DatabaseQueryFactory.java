...
public class DatabaseQueryFactory implements Serializable {
     ...
     */
    public DatabaseQueryFactory(DataSource source, boolean includeView) {
        dataSource = source;
    ...
    public DatabaseQuery newInstance() {
        try {
            //获取数据库URL 用于判断数据库类型（连接用完即还，避免泄漏连接池租借）
            String url;
            ...
            //优先使用 (DataSource, boolean) 构造（视图支持），否则回退 (DataSource)
            try {
                Constructor<? extends DatabaseQuery> constructor = query
                    .getConstructor(DataSource.class, boolean.class);
                return constructor.newInstance(dataSource, includeView);
